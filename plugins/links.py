# RoboCop 2's links.py - Universal links module extracts metadata from URLs to provide context.
...
def linkfinder(link):
    ...
    try:
        ...
        return "Invalid or malformed link."
    page = data.text
    soup = BeautifulSoup(page)
    meta1 = soup.find_all('meta',attrs={'name':'Description'})
    meta2 = soup.find_all('meta',attrs={'name':'description'})
    descr = "";
    ...
    title = re.sub('\s+',' ',soup.title.string)
    description = formatting.truncate_str(descr, 136)
    unknown = "Image or other non-HTML file."
    unkmsg = "\x02 Link: \x02 {}".format(unknown)

    if not descr:
        try:
            return "\x02 Link: \x02 '{}'".format(title)
        except AttributeError:
            return unkmsg
    else:
        try:
...
def linkmatcher(match, bot):
    ...
    if switch == "ON":
        response = linkfinder(match)
        return response

@hook.command
def preview(text):
    response = linkfinder(text)
    return response